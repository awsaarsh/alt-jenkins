...
function Register() {
  ...
  const fetchStates = async (country) => {
    try {
      const response = await axios.get(`${apiUrl}provinces-or-states/?country=${country}`);
      country === 'USA' ? setStates(response.data.states) : setStates(response.data.provinces);
    } catch (error) {
  ...
  const handleSubmit = async e => {
    ...
    if (Object.keys(errors).length === 0) {
      try {
        ...
        const response = await axios.post(`${apiUrl}register/`, formData, { headers });
